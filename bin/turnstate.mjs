...
async function main() {
  ...
  if (command === 'probe-start') {
    ...
    const body = has('input-stdin') ? JSON.parse(fs.readFileSync(0, 'utf8')) : {
      source: 'next_request', model: arg('model'), bindingId: arg('binding'), maxAttempts: Number(arg('attempts', '3'))
    };
