...
**节点维护与普通转发分离。** 不修改 sub2api 源码、数据库、账号 proxy_id 或正式请求出口。支持 HTTP CONNECT、HTTPS CONNECT 和 SOCKS5 用户密码认证。裸 `host:port:user:password` 默认为 HTTP；也支持 URL、空格分隔和“地址 + 用户密码”分行输入。每个节点可编辑、停用、删除、排序及检查实际公网出口。公开示例必须使用占位凭据。
