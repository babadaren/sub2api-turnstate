...
响应声明模型不符/冲突、HTTP 401/403，以及明确指向 turn-state 的结构化拒绝，仍使本次使用的绑定失效。`lib/failure-policy.mjs` 使用有限本地兼容规则：识别专用 state 错误码，或明确 state 参数与 invalid/expired 类型组合；不靠错误 message 的文字猜测。这里的错误码集合不是经过认证的完整上游错误目录，未知错误默认保留状态以避免误判，需要依据新日志核对。
