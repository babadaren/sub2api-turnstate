    ...
    try {
      request = http.request({ hostname: url.hostname, port: url.port || 80, path: endpoint, method: 'POST', headers, agent: false }, res => {
        ...
        res.on('data', chunk => {
          ...
          // Response model is diagnostic only. Length is the configured criterion;
          // a successful terminal event is still required before adopting a state.
          else if (!wanted) finish({ error: 'length_miss' });
          else if (wanted && (meta.completed || meta.outputLimitReached)) finish({ state, completed: meta.completed, accepted: true, modelConflict:!!meta.modelConflict });
        });
        res.on('end', () => {
          meta.end(); result.responseModel = meta.model;
          if (meta.failed) finish({ error: 'response_failed' });
          else if (!wanted) finish({ error: 'length_miss' });
          else if (meta.completed || meta.outputLimitReached) finish({ state, completed: meta.completed, accepted: true, modelConflict:!!meta.modelConflict });
