...
import { retryableProbeResult, validAttemptCount } from './retry-policy.mjs';
...
export function validatePreflight(value) {
  ...
  for (const [model, raw] of Object.entries(value)) {
    ...
    if (!validAttemptCount(r.maxAttempts)) throw new Error('maxAttempts must be a positive safe integer.');
    for (const [key, min, max] of [['maxWaitSeconds',5,3600],['intervalSeconds',2,10],['cooldownSeconds',10,600]]) {
...
export class PreflightGate {
  ...
  snapshot() {
    return { rules: this.rules, activeJobs: [...this.jobs.values()].map(j=>({...j.public,waiters:j.waiters})),
      ...
      budget: this.probes.budgetStatus(),
      limits: { maxJobs:2, maxWaitingRequests:8, maxHeldRequestBytes:67108864, maxAttempts:null, maxWaitSeconds:3600, maxAttemptsPerHour:this.probes.budgetLimit },
  ...
  async ensure(ctx, headers, payload, requestPath, bytes, signal) {
    ...
    if (!job) {
      ...
      const p = {id:randomUUID(),model:ctx.model,bindingId:ctx.id,status:'running',createdAt:started,
        deadline:started+rule.maxWaitSeconds*1000,attempts:0,maxAttempts:rule.maxAttempts,mismatchCount:0,lengthMissCount:0,nextAttemptAt:null,lastResult:null};
      ...
      job.promise = this.run(job,routing,requestPath).catch(()=>({ok:false,reason:'internal_error'})).then(result=>{
        p.status=result.ok?'found':result.reason; p.finishedAt=Date.now(); p.nextAttemptAt=null;
  ...
  async run(job,routing,requestPath) {
    ...
    for(let i=0;i<rule.maxAttempts;i++) {
      if(i>0) {p.nextAttemptAt=Date.now()+rule.intervalSeconds*1000;try{await delay(rule.intervalSeconds*1000,null,{signal});}catch{}}
      ...
      p.lastResult={status:out.status,length:out.length,responseModel:out.responseModel,outcome:out.error||'candidate',retryable:retryableProbeResult(out)};
      ...
      // Retry model/length misses without ever accepting the mismatching state's value.
      if(!retryableProbeResult(out))return {ok:false,reason:out.error||'not_accepted'};
    }
