...
export class ProxyPool {
  ...
  ready() {const p=this.data.profile;return !this.invalid&&!!p?.token&&!!p.allowedClientHash&&!!p.type&&(p.type!=='codex'||!!p.accountId);}
  snapshot() {
    const profile=this.data.profile;
    return {enabled:this.data.enabled,ready:this.ready(),configurationError:this.invalid,nextNodeId:this.data.nextId,
      profile:profile?{type:profile.type,tokenConfigured:!!profile.token,accountConfigured:!!profile.accountId,clientKeyConfigured:!!profile.allowedClientHash}:null,
      ...
      notice:'仅短探测使用导入节点；正式请求仍走 sub2api。独立探测必须单独配置真实上游认证，不可把 sub2api API Key 当作上游凭据。探测不进入 sub2api 计费记录。跨出口固定值兼容性未保证。'};
  }
  ...
  configureProfile(body) {
    ...
    if(body.clear===true){d.profile=null;d.enabled=false;}
      ...
      const old=d.profile||{},type=body.type??old.type;
  ...
  allowed(routing) {
    ...
    const actual=Buffer.from(this.hashClient(key)),expected=Buffer.from(this.data.profile?.allowedClientHash||'');
  ...
  async probe(jobId,model,routing,lengths,signal,fallback) {
    if(!this.data.enabled||!this.ready()||!this.allowed(routing))return {...await fallback(),probeRoute:{kind:'sub2api',reason:!this.data.enabled?'pool_disabled':!this.ready()?'source_missing':'client_not_bound'}};
    ...
    try {out=await this.transport({...node},{...this.data.profile},model,lengths,controller.signal,15);}
    ...
    const route={kind:'proxy',nodeId:id,nodeName:node.name};
