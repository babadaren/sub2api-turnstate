...
import { retryableProbeResult, validAttemptCount, validRunSeconds, MAX_RECENT_ATTEMPTS } from './retry-policy.mjs';
...
export class ProbeManager {
  constructor(config, states, journal, getMode) {
    ...
    this.limitsFile = path.join(path.dirname(journal.file), 'probe-limits.json');
    this.budgetLimit = 30;
    try {
      const saved = JSON.parse(fs.readFileSync(this.limitsFile, 'utf8'));
      if (!Number.isSafeInteger(saved.maxAttemptsPerHour) || saved.maxAttemptsPerHour < 1 || saved.maxAttemptsPerHour > 100000) throw new Error('Invalid hourly budget');
      this.budgetLimit = saved.maxAttemptsPerHour;
    } catch (e) { if (e.code !== 'ENOENT') throw new Error('Invalid probe-limits.json'); }
    try {
  ...
  consumeAttempt() {
    ...
    if(this.budgetUnavailable || this.attempts.length>=this.budgetLimit)return false;
  ...
  budgetStatus() {
    const now = Date.now(), active = this.attempts.filter(t => now - t < 3600000);
    return { maxAttemptsPerHour: this.budgetLimit, used: active.length,
      remaining: Math.max(0, this.budgetLimit - active.length),
      nextSlotAt: active.length ? active.reduce((min, t) => Math.min(min, t), Infinity) + 3600000 : null, unavailable: this.budgetUnavailable };
  }
  configureBudget(value, acknowledged = false) {
    if (!acknowledged) throw new Error('Explicit billable and experimental acknowledgement required.');
    const limit = value?.maxAttemptsPerHour;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100000) throw new Error('Hourly budget must be an integer in 1..100000.');
    if (this.automaticActive || BUSY.has(this.job?.public.status)) throw new Error('Stop active/waiting probes before changing the budget.');
    atomicJSON(this.limitsFile, { maxAttemptsPerHour: limit }); this.budgetLimit = limit;
    this.journal.add({ kind: 'admin', action: 'probe_budget_updated', maxAttemptsPerHour: limit });
    return this.budgetStatus(); // Never resets usage, including when increasing the limit.
  }
  remember(ctx) {
  ...
  snapshot() {
    ...
    return { job: this.job ? structuredClone(this.job.public) : null,
      ...
      limits: { maxAttempts: null, attemptsPolicy: 'positive_safe_integer', defaultMaxRunSeconds: 180, maxRunSeconds: 3600, minIntervalSeconds: 2, maxAttemptsPerHour: this.budgetLimit, maxStartsPerTenMinutes: 3, retainedAttempts: MAX_RECENT_ATTEMPTS },
  ...
  start(input) {
    ...
    if (this.starts.length >= 3 || this.budgetUnavailable || this.attempts.length >= this.budgetLimit || now - this.lastStart < 2000) throw new Error('Probe cooldown/budget reached; no requests sent.');
    ...
    const count = input.maxAttempts ?? 3, interval = input.intervalSeconds ?? 2, maxRunSeconds = input.maxRunSeconds ?? 180;
    if (!validAttemptCount(count) || !Number.isInteger(interval) || interval < 2 || interval > 10) throw new Error('Attempts must be a positive safe integer; interval must be 2..10 seconds.');
    if (!validRunSeconds(maxRunSeconds)) throw new Error('Total run time must be 5..3600 seconds.');
    ...
    const pub = { id: randomUUID(), model, source, endpoint, targetLengths: [...new Set(lengths)], maxAttempts: count, intervalSeconds: interval, maxRunSeconds,
      status: source === 'next_request' ? 'waiting_request' : 'running', createdAt: now, startedAt: null, deadline: now + 120_000,
      bindingId: source === 'next_request' ? input.bindingId : ctx.id, tried: 0, hits: 0, mismatchCount: 0, lengthMissCount: 0, nextAttemptAt: null, lastResult: null, results: [], finishedAt: null };
  ...
  launch(job, ctx, routing) {
    clearTimeout(job.timer); job.public.status = 'running'; job.public.startedAt = Date.now(); job.public.deadline = Date.now() + job.public.maxRunSeconds * 1000;
    job.timer = setTimeout(() => this.stop('deadline'), job.public.maxRunSeconds * 1000);
    this.pending = this.run(job, ctx, routing).catch(() => { if (BUSY.has(job.public.status)) job.public.status = 'internal_error'; }).finally(() => {
      clearTimeout(job.timer); job.public.finishedAt = Date.now(); job.public.nextAttemptAt = null;
  ...
  async run(job, ctx, routing) {
    ...
    for (let index = 0; index < p.maxAttempts; index++) {
      ...
      if (Date.now() >= p.deadline) { this.stop('deadline'); return; }
      if (index > 0) { p.nextAttemptAt = Date.now() + p.intervalSeconds * 1000; try { await delay(p.intervalSeconds * 1000, null, { signal }); } catch { return; } }
      ...
      const out = await probeOnce(this.config.target, p.model, routing, p.endpoint, p.targetLengths, signal, Math.min(15, (p.deadline - Date.now()) / 1000));
      if (signal.aborted) return;
      const result = { attempt: p.tried, time: Date.now(), status: out.status, length: out.length, responseModel: out.responseModel, outcome: out.error || 'target_candidate', retryable: retryableProbeResult(out) };
      ...
      if (p.results.length > MAX_RECENT_ATTEMPTS) p.results.shift();
      ...
      // Model routing misses can be sampled again; wrong-model values are never adopted.
      // Authentication, rate-limit, network, failed-event and 5xx errors remain terminal.
      if (!retryableProbeResult(out)) { p.status = out.error || 'not_accepted'; return; }
    }
    p.status = 'exhausted';
  }
  stop(reason = 'stopped') {
