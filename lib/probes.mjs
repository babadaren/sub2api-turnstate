...
function identity(headers, payload) {
  const safe = {};
    ...
    try {
      request = http.request({ hostname: url.hostname, port: url.port || 80, path: endpoint, method: 'POST', headers, agent: false }, res => {
        ...
        res.on('data', chunk => {
          ...
          else if (meta.model && meta.model !== model) finish({ error: 'model_mismatch' });
        ...
        res.on('end', () => {
          ...
          if (meta.failed) finish({ error: 'response_failed' });
          else if (meta.model !== model) finish({ error: meta.model ? 'model_mismatch' : 'response_model_missing' });
...
export class ProbeManager {
  ...
  async run(job, ctx, routing) {
    ...
    for (let index = 0; index < p.maxAttempts; index++) {
      ...
      if (signal.aborted) return;
      this.attempts.push(Date.now()); p.tried++;
