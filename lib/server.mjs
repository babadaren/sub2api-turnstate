...
import { ProbeManager } from './probes.mjs';
import { ResponseMetadata } from './response-meta.mjs';
import { PreflightGate } from './preflight.mjs';
...
export async function createExtension(config, home, options = {}) {
  ...
  const probes = new ProbeManager(config, states, journal, () => mode);
  const preflight = new PreflightGate(config,home,states,journal,probes,()=>mode);
  ...
  const setMode = value => {
    ...
    if (!['pin','observe'].includes(value)) probes.stop('processing_disabled');
    if(value!=='pin')preflight.cancel('processing_disabled');
  ...
  const proxy = http.createServer(async (req, res) => {
    ...
    const inspectBody = eligibleRoute && metadataReaders < config.metadataConcurrency;
    if (inspectBody) metadataReaders++;
    try {
      metadata = await requestMetadata(req, inspectBody, config.requestMetadataMaxBytes);
      if (eligibleRoute && !inspectBody) metadata.reason = 'inspection_busy';
    }
    ...
    if(gateRoute && metadata.complete && preflight.enabled(metadata.model)) {
      const controller=new AbortController(),abort=()=>controller.abort();
      res.once('close',abort);req.once('aborted',abort);
      try { gate=await preflight.ensure(gateContext,req.headers,metadata.payload,req.url,metadata.prefix?.length||0,controller.signal); }
      ...
      if(!gate.allow || res.destroyed) {
        ...
        journal.add({kind:'request',protocol:'http',method:req.method,path:routeName(req.url),model:metadata.model,
          ...
          status,client:clientID(req),mode,preflight:gate,requestBytes:metadata.prefix?.length||0,responseBytes:0,
          requestStateLength:typeof req.headers[HEADER]==='string'?Buffer.byteLength(req.headers[HEADER]):0,
          forwardedStateLength:0,responseStateLength:0,returnedStateLength:0,requestAction:'preflight_blocked',responseAction:'none',error:gate.reason});
        metadata.prefix=null;
        if(!res.destroyed)sendJSON(res,503,{error:{type:'turnstate_preflight_failed',message:'Required model state was not obtained; original request was NOT sent.',reason:gate.reason,attempts:gate.attempts||0}}, {'retry-after':String(gate.retryAfter||60),'x-turnstate-preflight':'blocked'});
        return;
      }
    ...
    if(gate.handled && gate.action==='failure_passthrough') {
      ...
      if(!pinned){sendJSON(res,503,{error:{type:'turnstate_preflight_failed',reason:'binding_changed',message:'Original request was NOT sent.'}});return;}
    ...
    if (eligible && currentMode !== 'off') probes.remember(context);
    ...
    const log = error => {
      ...
      journal.add({ kind: 'request', protocol: 'http', method: req.method,
        ...
        bindingId: context.id, client: clientID(req), mode: currentMode, preflight:gate, originalForwarded:true,
    ...
    const upstream = http.request({ protocol: 'http:', hostname: target.hostname, port: target.port || 80,
      ...
      responseReader = eligible && responseMetadataReaders < config.metadataConcurrency;
      ...
      if (!responseReader) responseMeta.skippedReason = 'inspection_busy';
      ...
      response.on('end', () => {
        responseMeta.end();
        if(eligible && currentMode!=='off')states.verifyResponsePin(context,response.headers[HEADER],responseMeta.model,
          status>=200 && status<300 && !responseMeta.failed && !responseMeta.modelConflict && (responseMeta.completed || responseMeta.outputLimitReached));
      });
    ...
    res.on('finish', () => {
      finished = true; log();
      if (eligible && status >= 200 && status < 300 && !responseMeta?.failed) probes.trigger(context, req.headers, metadata.payload);
    });
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/status' && req.method === 'GET') return sendJSON(res, 200, {
          ...health(), mode, active:active+preflight.waiters, startedAt, counters: journal.total, droppedLogRecords: journal.dropped,
          preflightSupported:true, preflightWaiting:preflight.waiters,
          target: config.target, csrf: session?.csrf, pinningSupported: true, probingSupported: true,
        ...
        if(url.pathname==='/api/preflight' && req.method==='GET')return sendJSON(res,200,preflight.snapshot());
        if(url.pathname==='/api/preflight/config' && req.method==='POST') {
          const body=await readJSON(req,65536);
          try{preflight.configure(body.rules,body.acknowledgeBillable===true && body.acknowledgeExperimental===true);return sendJSON(res,200,preflight.snapshot());}
          catch(error){return sendJSON(res,400,{error:error.message});}
        }
        if (url.pathname === '/api/probes' && req.method === 'GET') return sendJSON(res, 200, probes.snapshot());
        if (url.pathname === '/api/probes/budget' && req.method === 'GET') return sendJSON(res, 200, probes.budgetStatus());
        if (url.pathname === '/api/probes/budget' && req.method === 'POST') {
          const body = await readJSON(req);
          try { return sendJSON(res, 200, probes.configureBudget(body, body.acknowledgeBillable === true && body.acknowledgeExperimental === true)); }
          catch (error) { return sendJSON(res, 400, { error: error.message }); }
        }
        if (url.pathname === '/api/probes/start' && req.method === 'POST') {
          const body = await readJSON(req, 16384);
          try { return sendJSON(res, 200, probes.start(body)); }
          catch (error) { return sendJSON(res, 400, { error: error.message }); }
        }
        if (url.pathname === '/api/probes/stop' && req.method === 'POST') return sendJSON(res, 200, probes.stop());
        ...
        if (url.pathname === '/api/rules' && req.method === 'POST') {
          ...
          states.configure(body.rules); probes.stop('configuration_changed'); journal.add({ kind: 'admin', action: 'rules_updated' });
          preflight.cancel('configuration_changed');
          return sendJSON(res, 200, { ok: true, ...states.snapshot() });
        }
        if (url.pathname === '/api/pins/refresh' && req.method === 'POST') {
          const result = states.refresh(await readJSON(req)); probes.stop('binding_refreshed'); preflight.cancel('configuration_changed'); return sendJSON(res, 200, result);
        }
        ...
        if (url.pathname === '/api/pins/set' && req.method === 'POST') {
          ...
          states.manual(body.id, body.state); probes.stop('binding_replaced'); preflight.cancel('configuration_changed'); return sendJSON(res, 200, { ok: true });
        }
      ...
      const files = { '/': ['index.html', 'text/html; charset=utf-8'], '/app.js': ['app.js', 'text/javascript; charset=utf-8'], '/probe.js': ['probe.js', 'text/javascript; charset=utf-8'], '/preflight.js': ['preflight.js','text/javascript; charset=utf-8'], '/style.css': ['style.css', 'text/css; charset=utf-8'] };
  ...
  return { proxy, admin, journal, states, probes, preflight, get mode() { return mode; }, setMode,
    async close(graceMs = 3000) {
      await preflight.close(); await probes.close();
