...
export async function createExtension(config, home, options = {}) {
  ...
  const proxy = http.createServer(async (req, res) => {
    ...
    const log = error => {
      ...
      journal.add({ kind: 'request', protocol: 'http', method: req.method,
        ...
        bindingId: context.id, client: clientID(req), mode: currentMode,
    ...
    const upstream = http.request({ protocol: 'http:', hostname: target.hostname, port: target.port || 80,
      ...
      response.on('end', () => responseMeta.end());
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/pins/refresh' && req.method === 'POST') {
          const result = states.refresh(await readJSON(req)); probes.stop('binding_refreshed'); return sendJSON(res, 200, result);
        }
        ...
        if (url.pathname === '/api/pins/set' && req.method === 'POST') {
          ...
          states.manual(body.id, body.state); probes.stop('binding_replaced'); return sendJSON(res, 200, { ok: true });
        }
      ...
      const files = { '/': ['index.html', 'text/html; charset=utf-8'], '/app.js': ['app.js', 'text/javascript; charset=utf-8'], '/probe.js': ['probe.js', 'text/javascript; charset=utf-8'], '/style.css': ['style.css', 'text/css; charset=utf-8'] };
  ...
  return { proxy, admin, journal, states, probes, get mode() { return mode; }, setMode,
