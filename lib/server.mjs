  ...
  const proxy = http.createServer(async (req, res) => {
    ...
    const upstream = http.request({ protocol: 'http:', hostname: target.hostname, port: target.port || 80,
      ...
      if (processing()) {
        ...
        const pin = usedFingerprint && states.liveForPreflight(context), rule = context.rule;
      ...
      response.on('end', () => {
        ...
        if (processing()) {
          ...
          if(accepted && responseMeta.model===context.model && response.headersDistinct?.[HEADER]?.length===1) {
            ...
            states.captureAutomatic(context,raw,responseMeta.model);
          }
        }
      });
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/status' && req.method === 'GET') return sendJSON(res, 200, {
          ...
          notice: 'One switch: start automatic discovery and pinning, or stop and pass through unchanged. No attempt quota or overall probe deadline. A waiting real request supplies routing credentials; no original prompt enters the probe. Other models and uninspectable HTTP/WS traffic pass through.'
        });
