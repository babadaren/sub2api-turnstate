...
import { StateStore, validModel, DEFAULT_TTL_SECONDS, DEFAULT_SCOPE } from './states.mjs';
...
import { pinInvalidationReason } from './failure-policy.mjs';
...
export async function createExtension(config, home, options = {}) {
  ...
  const proxyPool = new ProxyPool(home,config,journal,options.proxyPoolOptions);
  const automatic = new AutomaticGate(config,states,journal,()=>mode==='auto',{proxyPool});
  ...
  const configureRules = rules => {
    const changed=states.configure(rules);
    for(const model of changed)automatic.cancelModels([model],!states.rules[model]?'model_removed':!states.rules[model].enabled?'model_disabled':'configuration_changed');
    journal.add({kind:'admin',action:'rules_updated',models:changed});
    return changed;
  };
  ...
  const proxy = http.createServer(async (req, res) => {
    ...
    let pinInvalidated = false, invalidationReason = null;
    const processing = () => eligible && currentMode === 'auto' && mode === 'auto' && processingRevision === modeRevision &&
      context.epoch===(states.epochs.get(context.model)||0) && context.ruleSignature===states.signature(context.model);
    ...
    const invalidateFromEvidence = () => {
      if (!processing() || !usedFingerprint || pinInvalidated) return;
      const reason = pinInvalidationReason(status,responseMeta,context.model);
      if (!reason) return;
      invalidationReason = reason;
      pinInvalidated = states.invalidateUsedPin(context,usedFingerprint,{
        expectedPin:usedPin,reason,requestRecordId,upstreamRequestId:responseMeta?.requestId,httpStatus:status,
        responseModel:responseMeta?.model,errorCode:responseMeta?.errorCode,errorType:responseMeta?.errorType,
        errorParam:responseMeta?.errorParam,incompleteReason:responseMeta?.incompleteReason
      });
    };
    active++;
    const log = error => {
      ...
      if (!eligible) return;
      journal.add({ id:requestRecordId, kind: 'request', protocol: 'http', method: req.method,
        ...
        responseModelConflict:!!responseMeta?.modelConflict,responseErrorCode:responseMeta?.errorCode || null,
        responseErrorType:responseMeta?.errorType || null,responseErrorParam:responseMeta?.errorParam || null,
        responseIncompleteReason:responseMeta?.incompleteReason || null,responseFailureEvent:responseMeta?.failureEvent || null,
        pinInvalidated,pinInvalidationReason:invalidationReason,usedPinFingerprint:usedFingerprint,
        pinDecision:!usedPin?'not_used':pinInvalidated?'invalidated':invalidationReason?'already_invalidated_or_replaced':
          !processing()?'processing_disabled':states.liveForPreflight(context)===usedPin?'retained':'expired_or_replaced',
    ...
    const upstream = http.request({ protocol: 'http:', hostname: target.hostname, port: target.port || 80,
      ...
      response.on('data', chunk => {
        ...
        if (eligible) responseMeta.push(chunk);
        // A structured rejection is acted on even if the stream later disconnects.
        invalidateFromEvidence();
      });
      response.on('end', () => {
        responseMeta.end();
        if (processing()) {
          const accepted = status>=200 && status<300 && !responseMeta.failed && (responseMeta.completed || responseMeta.outputLimitReached);
          invalidateFromEvidence();
          const raw=response.headers[HEADER];
          if(accepted && response.headersDistinct?.[HEADER]?.length===1) {
            // Length-based adoption retains the actual response model as metadata.
            states.captureAutomatic(context,raw,responseMeta.model,usedPin || undefined);
          }
          if(accepted && context.scope==='model')automatic.remember(context,req.headers,metadata.payload,req.url,true);
        }
      });
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/status' && req.method === 'GET') return sendJSON(res, 200, {
          ...
          target: config.target, csrf: session?.csrf, pinningSupported: true, acceptance:'length', modelControlsSupported:true,
          ...
          notice: 'Length-based acceptance for configured models; response model names are diagnostic, not rejection conditions. Disable/delete cancels only that model. Ten-minute early renewal, request binding isolation and transport errors remain unchanged.'
        });
        ...
        if (url.pathname === '/api/proxy-pool' && req.method === 'GET') return sendJSON(res,200,proxyPool.snapshot());
        if (url.pathname.startsWith('/api/proxy-pool/') && req.method === 'POST') {
          try {
            const body=await readJSON(req,65536);
            if(url.pathname==='/api/proxy-pool/nodes')return sendJSON(res,200,proxyPool.mutate(body));
            if(url.pathname==='/api/proxy-pool/source')return sendJSON(res,200,proxyPool.configureProfile(body));
            if(url.pathname==='/api/proxy-pool/switch')return sendJSON(res,200,proxyPool.setEnabled(body.enabled,body.acknowledgeExperimental===true));
            if(url.pathname==='/api/proxy-pool/check')return sendJSON(res,200,await proxyPool.check(body.id));
            return sendJSON(res,404,{error:'Not found'});
          } catch(e) { return sendJSON(res,400,{error:e.message==='Body too large'?'导入内容过大':/^[\u4e00-\u9fff]/.test(e.message||'')?e.message:'代理配置或连接操作失败'}); }
        }
        if (url.pathname === '/api/states' && req.method === 'GET') return sendJSON(res, 200, { ...states.snapshot(), mode });
        if (url.pathname === '/api/rules/model' && req.method === 'POST') {
          const body=await readJSON(req,16384), {model,action}=body;
          if(!validModel(model)||!['enable','disable','delete','save'].includes(action))return sendJSON(res,400,{error:'Invalid model/action'});
          if(['enable','save'].includes(action)&&body.acknowledgeExperimental!==true)return sendJSON(res,400,{error:'Length-based replay requires acknowledgement'});
          const rules=structuredClone(states.rules), existing=rules[model];
          if(action==='delete')delete rules[model];
          else if(action==='save')rules[model]={...(existing||{ttlSeconds:DEFAULT_TTL_SECONDS,scope:DEFAULT_SCOPE,unknownPolicy:'pass',autoLearn:false}),...body.rule};
          else {
            if(!existing)return sendJSON(res,404,{error:'Model rule not found'});
            rules[model].enabled=action==='enable';
          }
          const changed=configureRules(rules);
          journal.add({kind:'admin',action:'model_'+action,model});
          return sendJSON(res,200,{ok:true,model,action,changed,...states.snapshot()});
        }
      ...
      const files = { '/': ['index.html','text/html; charset=utf-8'], '/app.js': ['app.js','text/javascript; charset=utf-8'], '/proxy-pool.js': ['proxy-pool.js','text/javascript; charset=utf-8'], '/records.js': ['records.js','text/javascript; charset=utf-8'], '/style.css': ['style.css','text/css; charset=utf-8'] };
  ...
  return { proxy, admin, journal, states, automatic, proxyPool, get mode() { return mode; }, setMode,
    async close(graceMs = 3000) {
      await automatic.close();proxyPool.close();
