...
import { errorEvidence, safeIncompleteReason } from './failure-policy.mjs';
...
export class ResponseMetadata {
  constructor(headers = {}, limit = 65536) {
    ...
    this.errorCode = null; this.errorType = null; this.errorParam = null;
    this.incompleteReason = null; this.failureEvent = null; this.stateRejected = false;
  }
  reason() {
  ...
  object(value) {
    ...
    if (validModel(response.model)) {
      if(this.model && this.model!==response.model)this.modelConflict=true;
      if(!this.model)this.model=response.model;
    }
    if (value.type === 'error' || value.type === 'response.failed' || value.error || response.error || response.status === 'failed') {
      this.failed = true;
      const event = value.type === 'error' ? 'error' : value.type === 'response.failed' ? 'response.failed' : 'response_error';
      // Supports both flat SSE error events and nested Response.error objects.
      // Only enum-like fields survive; message/body content is not retained.
      for (const error of [value.error, response !== value ? response.error : null, value.type === 'error' ? value : null]) {
        const detail = errorEvidence(error);
        if (!detail) continue;
        if (detail.stateRejected || !this.errorCode || this.errorCode === 'other') {
          this.errorCode = detail.code; this.errorType = detail.type; this.errorParam = detail.param;
        }
        this.stateRejected ||= detail.stateRejected;
      }
    ...
    if (value.type === 'response.incomplete' || response.status === 'incomplete') {
      this.incompleteReason = safeIncompleteReason(response.incomplete_details?.reason);
      if (response.incomplete_details?.reason === 'max_output_tokens') this.outputLimitReached = true;
      else { this.failed = true; this.failureEvent ||= 'response.incomplete'; }
    }
  }
