...
export function routeName(rawUrl) {
  ...
  // Sub2API also accepts the unversioned /responses route used by Codex.
  // Recognize both aliases, without rewriting the upstream URL or logging IDs.
  const match = /^(\/(?:v1\/)?responses)(?:\/|$)/.exec(pathname);
  if (!match) return null;
  return pathname === `${match[1]}/compact` ? `${match[1]}/compact` : match[1];
}
