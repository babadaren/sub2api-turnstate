...
const cancelledReasons = new Set(['disabled','model_disabled','model_removed','client_disconnected','configuration_changed','service_stopped','binding_changed']);
...
export class AutomaticGate {
  ...
  remember(ctx, headers, payload, requestPath, confirmed=false) {
    if (!this.current(ctx) || !this.states.liveForPreflight(ctx)) return false;
    // Shared state cannot be used to replace the renewal credential with an
    // unverified caller's key. Arm only after a successful upstream response.
    if(ctx.scope==='model' && !confirmed)return false;
    const retained=this.renewals.get(ctx.id);
    if(ctx.scope==='model' && retained && retained.ctx.client!==ctx.client)return true;
  ...
  renewalSnapshot() {
    ...
    return [...this.states.pins.values()].map(pin => {
      ...
      const usable = this.states.qualified(pin);
  ...
  snapshot() {
    return { enabled:this.isEnabled(), activeJobs:[...this.jobs.values()].map(j => ({...j.public,waiters:j.waiters})),
      ...
      eligibleModels:Object.entries(this.states.rules).filter(([,r]) => r.enabled && r.pinLengths.length).map(([model,r]) => ({model,lengths:r.pinLengths,scope:r.scope,ttlSeconds:r.ttlSeconds})),
      acceptance:'length', notice:'Configured lengths and successful responses determine acceptance; response models are diagnostic only. Per-model disable/delete cancels only that model. Renewal starts ten minutes before a one-hour local expiry. Credentials are memory-only; length is not proof of model identity or upstream validity.' };
  }
  ...
  cancelModels(models, reason='configuration_changed') {
    const selected=new Set(models);
    // StateStore.configure already increments changed models' epochs. Leave the
    // global generation, other model jobs, and other renewal credentials intact.
    for(const job of this.jobs.values())if(selected.has(job.ctx.model)&&!job.controller.signal.aborted){job.reason=reason;job.controller.abort();}
    for(const [id,slot] of [...this.renewals])if(selected.has(slot.ctx.model)) {
      this.renewals.delete(id);
      const ctx=this.states.context(slot.ctx.model,slot.routing.headers,slot.routing.payload);
      if(reason==='configuration_changed' && this.current(ctx) && this.states.liveForPreflight(ctx) && !this.renewals.has(ctx.id)) {
        slot.ctx=structuredClone(ctx);this.renewals.set(ctx.id,slot);
      } else wipe(slot.routing);
    }
    for(const [key,cool] of this.cooldowns)if(selected.has(cool.model))this.cooldowns.delete(key);
  }
  tick() {
    ...
    for (const [id,slot] of this.renewals) {
      ...
      if (!this.states.qualified(pin)) { this.forget(id); continue; }
  ...
  launch(ctx, routing, requestPath, purpose) {
    ...
    pub.trigger=purpose==='renewal'?'expiry_window':!basePin?'missing_pin':basePin.pending?'pin_invalidated':basePin.expiresAt<=started?'pin_expired':'unqualified_pin';
    pub.triggerReason=basePin?.pending?basePin.invalidationReason||'unknown_prior_invalidation':null;
    ...
    this.journal.add({kind:'automatic',action:job.background?'renewal_started':'started',purpose,model:ctx.model,jobId:pub.id,bindingId:ctx.id,trigger:pub.trigger,triggerReason:pub.triggerReason});
    job.promise=this.run(job,routing,requestPath).catch(()=>({ok:false,reason:'internal_error'})).then(result=>{
      ...
      if(result.ok && job.generation===this.generation && this.current(ctx)) {
        if(!this.renewals.has(ctx.id))this.remember(ctx,routing.headers,routing.payload,requestPath,true);
        ...
        const cooldownKey=ctx.scope==='model' && ['http_401','http_403'].includes(result.reason)?`${key}:auth:${ctx.client}`:key;
        this.cooldowns.set(cooldownKey,{reason:result.reason,until,model:ctx.model});
      ...
      return result;
    }).finally(()=>{this.jobs.delete(key);this.proxyPool?.endJob(pub.id);wipe(routing);});
    return job;
  }
  async ensure(ctx, headers, payload, requestPath, bytes, signal) {
    ...
    const started=this.now(),generation=this.generation,key=this.key(ctx),cool=this.cooldowns.get(`${key}:auth:${ctx.client}`)||this.cooldowns.get(key);
    ...
    try {
      ...
      if(['disabled','model_disabled','model_removed'].includes(job.reason)||!this.isEnabled())return {allow:true,handled:false,action:'disabled',reason:job.reason,...detail};
  ...
  async run(job,routing,requestPath) {
    ...
    while(!signal.aborted) {
      ...
      const fallback=()=>this.probe(this.config.target,ctx.model,routing,endpoint,ctx.rule.pinLengths,signal,15);
      const out=this.proxyPool?await this.proxyPool.probe(p.id,ctx.model,routing,ctx.rule.pinLengths,signal,fallback):await fallback();
      ...
      const qualified=out.status>=200&&out.status<300&&!out.error&&out.accepted&&out.state&&ctx.rule.pinLengths.includes(out.length);
      ...
      p.lastResult={status:out.status,length:out.length,responseModel:out.responseModel,outcome,modelReportedDifferent:!!out.responseModel&&out.responseModel!==ctx.model,acceptance:'length',probeRoute:out.probeRoute||{kind:'sub2api'}};
      Object.assign(p.lastResult,{responseContentType:out.responseContentType||null,responseEncoding:out.responseEncoding||null,responseDetection:out.responseDetection||null});
      ...
      if(qualified&&!unchanged) {
        ...
        this.states.pins.get(ctx.id).probeRoute=out.probeRoute||{kind:'sub2api'};
        this.states.flush();this.totals.found++;if(job.background)this.totals.renewed++;
        return {ok:true};
      }
      if(!out.retryProxy && !(out.status>=200&&out.status<300&&['model_mismatch','length_miss','unchanged_state'].includes(outcome)))return {ok:false,reason:out.error||'not_accepted',retryAfterMs:out.retryAfterMs};
