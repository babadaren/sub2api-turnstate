...
let authenticated=false,enabled=false,csrf='',loading=false,switchBusy=false,rules={},serverOffset=0,revealed=null,renewalById=new Map(),recordsPager=null,proxyPanel=null;
...
function showLogin(){authenticated=false;enabled=false;csrf='';forget();$('dashboard').hidden=true;$('login').hidden=false;$('badge').textContent='未登录';$('pins').replaceChildren();recordsPager?.reset();proxyPanel?.reset();}
...
proxyPanel=createProxyPoolPanel({api,notice:note});
