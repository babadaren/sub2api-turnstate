...
function renderRecordsPage(records,page){
  if(records){
    const rows=records.map(r=>{const tr=document.createElement('tr');
      ...
      cells(tr,[time(r.time),r.kind==='request'?`${r.method} ${r.path}`:`${r.kind} / ${r.action}`,model,r.status,
        ...
        [r.requestAction,r.automatic?.action,r.outcome,r.error].filter(Boolean).map(v=>names[v]||v).join(' / ')]);return tr;});
