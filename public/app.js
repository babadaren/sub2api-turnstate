...
$('rule-form').onsubmit=async e=>{e.preventDefault();try{const f=e.target.elements,m=f.model.value.trim(),list=s=>s.split(/[,，\s]+/).filter(Boolean).map(Number);const r={enabled:f.enabled.checked,pinLengths:list(f.pinLengths.value),discardLengths:list(f.discardLengths.value)};await api('/api/rules/model',{model:m,action:'save',rule:r,acknowledgeExperimental:true});e.target.hidden=true;note('模型规则已保存；启动/关闭状态未改变。');await refresh();}catch(err){note(err.message);}};
